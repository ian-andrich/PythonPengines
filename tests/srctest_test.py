...
class PenginesTestCase(unittest.TestCase):
    ...
    def test_member(self):
        ...
        """
        q = "member(X,[1,2,3])"
        factory = PengineBuilder(urlserver="http://localhost:9083", destroy=False, ask=q)
    ...
    def test_src(self):
        ...
        src = "foo(a).\nfoo(b).\nfoo(c)."
        q = "foo(X)"
        factory = PengineBuilder(urlserver="http://localhost:9083", destroy=False, srctext=src, ask=q)
        pengine = Pengine(builder=factory, debug=True)
        results = pengine.currentQuery.availProofs
        print(results)
        self.assertTrue(len(results) == 3)
    ...
    def test_iterator(self):
        ...
        for chunk in chunk_sizes:
            factory = PengineBuilder(urlserver="http://localhost:9083", destroy=False, chunk=chunk, ask=q)
            pengine = Pengine(builder=factory, debug=True)
            results = []
